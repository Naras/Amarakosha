...
def make_script_maps():
    ...
    for i in range(9):
        ...
        for ch in range(0xA0, 0xFF + 1):
            ...
            if (ch in range(0xEB, 0xF1) or ch in range(0xFB, 0xFF + 1)):
                continue
...
class Parser:
    def __init__(self):
        ...
        self.prev_char, self.src_char, self.dest_char = NO_CHAR, NO_CHAR, NO_CHAR
    ...
    def handle_ext(self, curr_char):
        ...
        for a in range(1):
            ...
            if self.isvalid(dest_char):
                return dest_char
        logging.error("Invalid input after EXT %s" % (hex(i)))
        return None
    def handle_atr(self, i):
        if i in ISCII_SCRIPTS.keys():
            self.set_script(ISCII_SCRIPTS[i])
            logging.info("Handling ATR: setting script to " + ['devanagari','bengali','gurmukhi','gujarati','oriya','tamizh','telugu','kannada','malayalam'][i])
        else:
            # ignore all other ATR markers
            logging.info("Handling ATR: ignored")
            pass
    ...
    def iscii2utf8(self, src, flush=0):
        ...
        curr_char, prev_char = NO_CHAR, NO_CHAR
        n = len(src)
        if n == 0: return self.pos
        self.pos = 0
...
if __name__ == '__main__':
    ...
    try:
        ...
        i = int(sys.argv[1])
        if i not in range(1, 10): raise ValueError
